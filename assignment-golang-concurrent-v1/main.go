...
func (sm *InMemoryStudentManager) Login(id string, name string) (string, error) {
	...
	for _, student := range sm.students {
		if student.ID == id && student.Name == name {
			...
			studyProgram := sm.studentStudyPrograms[student.StudyProgram]
			return fmt.Sprintf("Selamat datang %s! Kamu terdaftar di program studi: %s.", student.Name, studyProgram), nil
		}
	}
